      ...
      <div class="filter-market">
        ...
        <div class="market-cat">
          <select class="market-list" v-model="market" @change="getCompanies">
            ...
            <option
              class="list-option"
              v-for="(mark, index) in marketsList"
              :key="index"
              :value="mark"
            >
              {{ mark }}
        ...
        <div class="market-comp">
          <select class="market-list" v-model="company" @change="setMarket">
            ...
            <option
              class="list-option"
              v-for="(comp, index) in companiesList"
              :key="index"
              :value="comp"
            >
              {{ comp }}
  ...
  methods: {
    async getCompanies() {
      ...
      if (this.market == "Todos") {
        ...
        emp = (
          await this.$http.get(`company?rubro=${this.market}`, {
            headers: {
              authorization: this.token,
            },
          })
    ...
    async getData() {
      let month = "",
        ...
        rubro = "",
        empresa = "";
      if (this.months != 100) month = "&month=" + this.months;
      if (this.items.length == 1) tipo = "&items=" + this.items[0];
      if (this.market != "Todos") rubro = "rubro=" + this.market;
      if (this.company != "Todas") empresa = "empresa=" + this.company;
      const response = (
        await this.$http.get(
          `quotation?type=criteria` +
            month +
            `&from=depto&nombre=${localStorage.getItem("depto")},` +
            tipo +
            rubro +
            empresa,
          {
      ...
      for (let i = 0; i < response.length; i++) {
        ...
        this.filteredInbox[i].fecha_cotizacion = this.filteredInbox[
          i
        ].fecha_cotizacion.substr(
          5,
          this.filteredInbox[i].fecha_cotizacion.indexOf("T")
        );
        this.filteredInbox[i].fecha_cotizacion = this.filteredInbox[
          i
        ].fecha_cotizacion.substr(
        ...
        const reqItems = (
          await this.$http.get(
            `itemsPerRequest?searchby=solicitud&typeinput=nombre&inputdata=${this.filteredInbox[i].titulo_cotizacion}`,
            {
              headers: {
                authorization: this.token,
              },
